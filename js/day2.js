...
let results = input2.filter(inposition);
document.write(results.length);


// for (line of input2) {
//     console.log(countem(line));
// }

function sussout(line) {
    let bits = line.split(" ");
    let pos = bits[0];
    let pwchar = bits[1].charAt(0);
    let pwstr = bits[2];

    // get the two numbers from pos
    let startEnd = pos.split("-");
    
    // construct the RegEx
    let re = new RegExp("[" + pwchar + "]{"+startEnd[0]+","+startEnd[1]+"}");
    console.log(re);
    // test 

    return re.test(pwstr);
}

function countem(line) {
    let bits = line.split(" ");
    let pos = bits[0];
    let pwchar = bits[1].charAt(0);
    let pwstr = bits[2];
    ...
    // get the two numbers from pos
    let startEnd = pos.split("-");
    let min = parseInt(startEnd[0]);
    let max = parseInt(startEnd[1]);
    
    // count how many times the char appears in the string
    
    let policycount = (pwstr.match(new RegExp(pwchar,"g")) || []).length;

    return (min <= policycount && policycount <= max);
}

function inposition(line) {
    let bits = line.split(" ");
    let pos = bits[0];
    let pwchar = bits[1].charAt(0);
    let pwstr = bits[2];
    ...
    let first = 1*(pwstr.charAt(min-1) == pwchar);
    let second = 1*(pwstr.charAt(max-1) == pwchar);
