...
let bagCount = 0;
 ...
 console.log("bags inside shiny gold: " + luggage["shiny gold"].countBagsInside());

/** #### FUNCTIONS #### */

function createBagNode_2(phrase) {
    // split the phrase up

    let h = phrase.split(/ contain |, /)
    for (let i = 0; i < h.length; i++) {
        h[i] = h[i].replace(/ bag[s]*[\.]*/,"")
    }
    ...
    // now the children are what's left of h
    let children = h[1]; // could be more than one
    
    DEBUG && console.log("shifted the node from h:");
    DEBUG && console.log(h);
